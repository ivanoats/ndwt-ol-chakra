 ...
 * loader has handed the full Site[] over via the page tree. Lookups
 * are O(1) via a Map keyed by SiteId.
 */
export class InMemorySiteRepository implements SiteRepository {
  private readonly byId: ReadonlyMap<string, Site>;

  constructor(private readonly sites: readonly Site[]) {
    this.byId = new Map(sites.map((site) => [site.id, site]));
  }
